...
const getPostDate = (id: string): string =>
...
export const getPosts = async (
  ...
  getCollection(
    'post',
    ({ id, data }) =>
      (lang ? id.startsWith(`${lang}/`) : true) &&
      (import.meta.env.PROD ? !data.draft : true)
  )

export const withSortedByDate = (
  posts: CollectionEntry<'post'>[]
): CollectionEntry<'post'>[] =>
  posts.sort((pre, cur) =>
    getPostDate(cur.id).localeCompare(getPostDate(pre.id))
  )

export const withGroupedByYear = (
  posts: CollectionEntry<'post'>[]
